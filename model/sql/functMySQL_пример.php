...
//модуль в котором реализованы функции работы с базой данных. Они собраны в один класс
...
//----------------------------------------------------------------------------------------------------------------   
    ...
    public function insertImage($objInsert, &$report) {
        ...
        if ($this->commands($queryPict
                        ...
                        , $report)) {
            $objInsert->idPict = mysqli_insert_id($this->link);
            return true;
        } else {
            return false;
        }
    ...
    public function selectImagePicture($id, &$report) {
        ...
        if ($result === false) {
            $report = $report . "<br/>picture: ошибка при чтении " . mysqli_error($this->link) . "\n";
            return false;
        }
...
//------------------Вставляет статью в базу данных---------------------------------------------  
    public function insertArticle($objInsert, &$report) {
        ...
        if ($objInsert->picture !== "") {//если картинка существует
            if ($this->insertImage($objInsert, $report) === false) {
                return false;
            }
        }
        if ($objInsert->idPict === 0) {
            $idPict = "NULL";
        } else {
            $idPict = "'$objInsert->idPict'";
        }
        $report = '';
        ...
        if ($this->commands($query
                        ...
                        , "info: ошибка записи "
                        , $report)) {
            $objInsert->id = mysqli_insert_id($this->link);
        } else {
            return false;
        }
        return true;
    ...
    public function updateArticle($objInsert, &$report) {
        ...
        if (($objInsert->picture !== "")and ( $objInsert->idPict == 0)) {//если картинки не было -вставляем
            ...
            if ($this->updateImage($objInsert, $report) === false) {
                return false;
            }
        } elseif (($objInsert->picture === "")and ( $objInsert->idPict !== 0)) {//удаляем картинку если она была
            if ($this->deleteImage($objInsert->idPict, $report) === false) {
                return false;
            }
        }
        $report = '';
...
//-------функция выводит false - ошибка запроса;""-не найдено ни одного подходящего пользователя;"права пользователя" в случае успеха;   
    ...
    public function getCountButtons(&$report) {
        ...
        if ($res === false) {
            $report = $report . "info: ошибка при чтении COUNT(*)" . mysqli_error($this->link) . "\n";
            return false;
        }
...
//----------------достает статью вместе с картинкой(используется для обновления объекта при переходе на редактирование)------------------------------------------------------
    public function selectOneNewsRefresh($id, &$outRes, &$report) {
        ...
        if ($outRes->idPict !== NULL) {
            $outRes->picture = $this->selectImagePicture($outRes->idPict, $report);
        } else {
            $outRes->picture = NULL;
        }
